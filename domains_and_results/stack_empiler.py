...
def o_place_precond(state, agent, cube_name, place_to):
...
def o_place_effects(state, agent, cube_name, place_to):
    ...
    if cube_can_be_placed(state, cube_name):
        for l in state.solution:
            if state.color_cubes[cube_name]["color"] in state.solution[l]["colors"] and target_supports_ok(state, l) and target_free(state, l):
                multi_subtasks.append([ ("place", cube_name, l) ])
    else:
...
def cube_reachable_by(state, cube, agent):
    ...
    return cube_side_reachable_by(state, cube, agent) and state.color_cubes[cube]["below"]==None
...
def initDomain():
    ...
    #     "l2" : {"colors":["g"], "on":["table"]},
    #     "l3" : {"colors":["p"], "on":["l1","l2"]},
    #     "l4" : {"colors":["b"], "on":["l3"]},
    #     "l5" : {"colors":["w"], "on":["l3"]},
    # })
    initial_state.create_static_fluent("solution", {
        "l1" : {"colors":["b"], "on":["table"]},
        "l2" : {"colors":["w"], "on":["table"]},
    })
